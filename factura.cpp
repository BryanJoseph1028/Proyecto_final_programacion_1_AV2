...
//VARIABLES PARA EL FUNCIONAMIENTO DEL PROGRAMA
    ...
    do{
    	srand(time(0));
	for(int i=0; i<1; i++){
	gotoxy(1,1);	correlativo= 1+(rand());  cout<<"Factura No."<<correlativo; cout<<"Factura No."<<correlativo<<"                   ";
	}
	//INICIO FECHA//
	time_t tiempoahora;	
	time(&tiempoahora);
	...
	gotoxy(8,1); cout<<"Fecha: "; printf("%d/%d/%d", mitiempo->tm_mday, mitiempo->tm_mon+1,mitiempo->tm_year+1900); //FIN FECHA
				 cout<<"Fecha: ";
				  cout<<("%d", mitiempo->tm_mday)<<"/"<<("%d",mitiempo->tm_mon+1)<<"/"<<("%d",mitiempo->tm_year+1900)<<endl;
        gotoxy(1,2); cout<<"NIT: ";
