...
int main(){
    ver_productos();
    int cp;
    cout<<"INGRESE CODIGO DE PRODUCTO A IMPRIMIR: ";
    cin>>cp;
	...
	fread ( &prod, sizeof(Productos), 1, archivo );
		do{
			if (prod.cod_producto == cp){
                ofstream archivo2;
                archivo2.open("IMPRIMIR/CODIGO_DE_BARRAS/CODIGO_DE_BARRAS/bin/Debug/imprimir.txt",ios::out);
                if(archivo2.fail()){
	...
	fclose(archivo);
	system("start IMPRIMIR/CODIGO_DE_BARRAS/CODIGO_DE_BARRAS/bin/Debug/CODIGO_DE_BARRAS.exe");
	...
	system("PAUSE");
	remove("IMPRIMIR/CODIGO_DE_BARRAS/CODIGO_DE_BARRAS/bin/Debug/imprimir.txt");
